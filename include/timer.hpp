...
private:
    uint64_t micros_array[MAX_TIMER_RECORD];
    uint64_t timer_micros[MAX_TIMER_RECORD];
    uint64_t timer_count[MAX_TIMER_RECORD];
    uint64_t timer_count_additional[MAX_TIMER_RECORD];
...
public:
    ...
    void StartTimer(TimerStep step) {
        micros_array[step] = NowMicros();
    }

    void Record(TimerStep step) {
        assert(micros_array[step] != 0);
        timer_micros[step] += NowMicros() - micros_array[step];
        timer_count[step]++;
    }
