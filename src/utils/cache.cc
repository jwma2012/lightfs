...
struct LRUHandle {
  ...
  LRUHandle* prev;
  size_t charge;      // TODO(opt): Only allow uint32_t? //// 所占内存大小
  size_t key_length;
...
HandleTable只有3个成员变量： uint32_t length_;   //hash表长，相当于二维数组的元素个数
  ...
  void Resize() {
    ...
    for (uint32_t i = 0; i < length_; i++) {
      LRUHandle* h = list_[i];//list_初始化时为NULL
      while (h != NULL) {
...
class LRUCache {
 public:
  ...
  size_t TotalCharge() const {
    MutexLock l(&mutex_);  //策略锁，按对象管理，自动析构
    return usage_;
  }
...
Cache::Handle* LRUCache::Insert(
  ...
  if (capacity_ > 0) {
    ...
    //插入到头部，同时删除table的尾部元素，但内存区没有free??
    //table_.Insert返回的是相同hash地址的一个元素
...
//FinishErase的作用是将该元素替换出缓存？?
//LRUCache是使用双向链表和hashtable组成的
...
static const int kNumShards = 1 << kNumShardBits;//16片
...
class ShardedLRUCache : public Cache {
 private:
  ...
  static uint32_t Shard(uint32_t hash) {
    return hash >> (32 - kNumShardBits);//得到一个4位数，0-15
  }
